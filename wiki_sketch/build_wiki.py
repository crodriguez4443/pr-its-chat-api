...
def extract_sp_code(title):
    ...
    The code may be preceded by start-of-string, an underscore (stakeholder-scoped
    titles like 'mpSH1_DM01-01'), or whitespace — a canonical title reads
    'Service Package DM01-01(MaineDOT)', so the space form is the common one.
    """
    m = re.search(r'(?:^|_|\s)([A-Z]{2,4}\d{2})', title)
    if m:
        code = m.group(1)
...
def build_traceability(data):
    ...
    for d in data:
        ...
        if not code:
            continue
        cat = extract_sp_category(code)
        # Two page templates carry the interface table: the full one heads it
        # "Interfaces and Standards", the abbreviated one "List of Interfaces".
        # Take whichever appears first.
        idx, heading = -1, None
        for h in ('Interfaces and Standards', 'List of Interfaces'):
            k = d['content'].find(h)
            if k >= 0 and (idx < 0 or k < idx):
                idx, heading = k, h
        if idx < 0:
            continue
        seg = d['content'][idx + len(heading):]
        # The full template continues into a projects table; cut it so those rows
        # can't be parsed as interfaces. The abbreviated template has no trailing
        # section, so find() returns -1 and the segment is used as-is.
        _end = seg.find('Projects Associated with this Service Package')
        if _end >= 0:
            seg = seg[:_end]
